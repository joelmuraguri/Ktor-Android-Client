...
class CoinsRepository(
    ...
    override suspend fun getCoinDetails(coinId: String): Flow<ResourceResult<CoinDetails>>  = flow{(
        emit(ResourceResult.Loading()))
        try {
            val response = service.getCoinDetails(coinId)
            emit(ResourceResult.Success(response.toCoinDetailsDomain()))
        } catch ( e: IOException){
            emit(ResourceResult.Error(message = "Couldn't reach Server. Check your Internet Connection"))
        }
    }
